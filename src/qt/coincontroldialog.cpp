                                                        ...
                                                        model(0)
{
    ...
    ui->labelTitle->setText("Select ALTC Denominations to Spend");
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Altecoin address. In tree mode, address is not shown again for direct wallet address outputs
