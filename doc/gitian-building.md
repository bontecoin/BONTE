...
*Setup instructions for a gitian build of Bontecoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Bontecoin
...
to the Bontecoin GitHub release page.
...
- [Building Bontecoin Core](#building-bontecoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/bontecoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/bontecoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/bontecoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/bontecoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/bontecoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/bontecoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/bontecoin-bonte/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@bontecoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/bontecoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/bontecoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/bontecoin-osx-*-build.assert
```
